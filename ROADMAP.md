...
- **0.18 Decomposition finish + batch-run visibility** - finish retiring `_logic.py`, then add live progress + running cost to the batch commands and a `-q`/`-v` verbosity dial while the CLI contracts are still pre-1.0. [detail](#0180--decomposition-finish--batch-run-visibility)
- **0.19 Recurring research profiles + no-metered-cost routing** - make recurring topics like "AI developer news" or "live agentic dev" first-class profiles, and add a cost policy that can refuse metered API calls while allowing local models or explicitly configured plan-quota CLIs such as Codex CLI, Claude Code, and Grok Build when they pass eval. [detail](#0190--recurring-research-profiles--no-metered-cost-routing)
...
### 0.18.0 - Decomposition finish + batch-run visibility

This closes the last codebase-legibility and human-loopability gap before 1.0.

- **Finish `_logic.py` removal.** Move `topic_app`, `topic_watch_app`, the remaining root callback body, and helper bodies into owned modules. Delete the facade only after call sites and patch strings are repointed, and keep the module-size ratchet must-only-decrease.
- **Batch progress and running cost.** Add consistent phase/item/completed/failed/spend output to `papers`, `discover`, `latest`, `catch-up`, and `site-batch`; preserve `--json` stdout purity by sending human progress to stderr.
- **Verbosity dial.** Add consistent `-q` / `-v` behavior before the CLI contract freezes.
- **Help examples.** Normalize `--help` examples for recurring workflows: preview, approve, ingest, audit, export OKF, and run from a next-action plan.
...
- **0.19.3 cross-route quality eval.** Add fixture comparisons for local sunk-cost routes, plan-quota CLI routes, and metered API routes. Use LLM-as-judge scoring over receipts so fan-out routing is based on output quality, not just availability.
...
- **0.19.5 profile run and loop handoff.** Enable approved profile refreshes and next-action JSON so cron, GitHub Actions, Codex, Claude Code, Grok Build, or a human operator can steward the profile without scraping console text.
