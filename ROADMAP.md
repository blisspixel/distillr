...
- **0.19.5 profile run and loop handoff.** Status: `distill profile run <name>` emits JSON plans, requires `--yes` to execute, records per-command outcomes, persists resume state, and emits profile-related next-action rows compatible with the audit action schema so cron, GitHub Actions, Codex, Claude Code, Grok Build, Gemini CLI, Antigravity, or a human operator can steward the profile without scraping console text. Remaining: audit-visible profile health.
