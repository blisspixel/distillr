...
Shipped: **0.1 through 0.19** (latest release 0.19.67, 2026-08-20). Per-release detail is the changelog's job, not the roadmap's: [`docs/CHANGELOG.md`](docs/CHANGELOG.md). Newest-first headlines:
...
- CLI flags, MCP tool/resource/prompt schemas, library directory layout, and frontmatter fields are versioned. Breaking changes require a major-version bump and a documented migration.
  - Status 2026-08-01: covered snapshots are **freeze-ready** (`status: freeze-ready` in `docs/contracts/*-v1.json`). The MCP 2026-07-28 checkpoint that blocked freeze readiness is complete (0.19.47 + 0.19.48). Compatibility and library corpus promises live in [`docs/contracts/COMPATIBILITY.md`](docs/contracts/COMPATIBILITY.md). Product names are frozen: CLI `distill` / `distill-mcp`, package `distillr`. Remaining uncovered slices (router env surface, artifact-specific schemas, full legacy migration automation) stay additive after freeze. Operational plan: [`docs/design/path-to-1.0.md`](docs/design/path-to-1.0.md).
- Documented backwards-compatibility policy for the `library/` directory (a 0.5 corpus opens cleanly in 1.0).
  - Status 2026-08-01: published in [`docs/contracts/COMPATIBILITY.md`](docs/contracts/COMPATIBILITY.md).
- **Measured performance baseline published.** The full contract is in [`docs/design/performance-and-language-admission.md`](docs/design/performance-and-language-admission.md), with the executable backlog in [`docs/roadmap.md`](docs/roadmap.md#9c-measured-performance-and-implementation-boundaries).
  ...
  - Status 2026-08-20: published offline baselines are the [`docs/performance/cross-platform-0.19.66.md`](docs/performance/cross-platform-0.19.66.md) Linux/macOS matrix and replay, [`docs/performance/baseline-0.19.60.md`](docs/performance/baseline-0.19.60.md) Windows scale-100/500/1_000/10_000 evidence, [`docs/performance/workflow-replay-0.19.63.md`](docs/performance/workflow-replay-0.19.63.md) Windows paper/video/site/synthesis/verify/profile/report replay, and the earlier [`docs/performance/baseline-0.19.50.md`](docs/performance/baseline-0.19.50.md) scale-100 receipt. The 0.19.66 run verified identical deterministic outputs across Linux and macOS after its first run exposed and drove a search-order fix. Building at least five comparable runs per host, adding installation and cold-start evidence, and running live reference journeys remain open. No native language admission.
...
- **Branch test coverage ≥95%**, ratcheted. 0.8.3 turns on branch coverage and starts the up-only climb from the measured baseline; the blocking CI and pre-push gates now enforce 95% across the surface. Branch (not line) is the metric, and the target is flat rather than tiered - the cost is real on presentation-heavy code (CLI rendering, web routes, dashboards), and that trade-off is accepted deliberately rather than hidden behind a per-package carve-out. Coverage is reported on every PR and can go up, not down.
  ...
  - Status 2026-07-31: the 0.19.47 local release gate passes 6,391 tests with
    four platform skips and eight live-network tests deselected at 95.05
    percent branch coverage, alongside ruff, pyright, import-linter, bandit,
    and both generated-contract checks. The release completes the MCP
    2026-07-28 compatibility inventory and ships the v1-SDK spec alignment:
    complete tool behavior hints held to the write-tool registry, sorted
    deterministic listings, distillr-version server identity, and the
    adversarial-review corrections recorded in the changelog.
  - Status 2026-07-31: the 0.19.48 local release gate passes 6,394 tests with
    four platform skips and eight live-network tests deselected at 95.05
    percent branch coverage, alongside ruff, pyright, import-linter, bandit,
    and both generated-contract checks. The release graduates the MCP SDK to
    the stable v2 line: the server negotiates protocol 2026-07-28 with
    modern clients and answers legacy initialize clients from the same stdio
    binary, proven against both client generations with the MCP contract
    snapshot byte-identical across the SDK swap. It also closes the MCP
    path-hygiene and stored-URL allowlist refinement debt (library-relative
    tool results; `catch_up` re-checks watch hosts).
  - Status 2026-07-31: the 0.19.49 local release gate passes 6,395 tests with
    four platform skips and eight live-network tests deselected at 95.04
