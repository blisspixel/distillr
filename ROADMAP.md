...
- **OS support matrix: Linux, macOS, and Windows are all first-class.** Today the full CI matrix runs ubuntu-only while development happens on Windows — so the cross-platform claim is real in practice but unenforced in CI. 1.0 adds macOS and Windows CI jobs (install + unit suite + CLI smoke at minimum; full matrix on ubuntu) so path handling, console rendering, and subprocess behavior are proven on every platform users actually run, not just the one CI happens to use. This tool has to work for anyone, on whatever box they have — including local models on consumer GPUs.
