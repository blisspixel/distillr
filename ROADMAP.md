...
- Preview-as-primary-flow UX: probe the candidate pool, detect the rerank-score cliff, present "top N excellent / top M including good / everything ≥ threshold" sizing options with per-option spend, then a single typed approval. *(Partial, 0.8.12: the score-cliff sizing line and a pre-run cost estimate now print under every preview shortlist. The full preview-as-default-on-fresh-topic flow with per-sizing-option spend remains a follow-on.)*
- Rerank determinism: *(Partial, 0.8.12.)* the discover rerank LLM call now runs at `temperature=0` for a reproducible previewed order; the cached commit-by-ID shortlist replay remains a follow-on.
- Real cost estimator that reads candidate metadata before the run (arXiv abstract length + page count; yt-dlp duration; site content-length) and calibrates against historical `cost_log.jsonl`. *(0.8.12 ships a count-based `estimate_discover_cost` from free metadata; the duration/length-aware calibration against `cost_log.jsonl` is the follow-on.)*
- `--rigor strict|balanced|loose` knob across discover/papers/latest. Audit and document the prompt divergence between commands. *(Shipped 0.8.12 for `discover`: a goal-fit threshold (0.7 / 0.5 / 0.3) drops weak candidates before the per-source limits. Extending the knob to papers/latest is a follow-on.)*
