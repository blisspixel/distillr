...
- **Engineering legibility + CLI-UX (0.13 onward, in progress)** - the run between the named source/trust/compounding milestones and 1.0. *Shipped:* the entailment tier + verify-on-every-synthesis (0.13), agent-grade `--json` / stdout discipline (0.14), `distill update` (0.15), the blocking golden-corpus eval gate (0.16), progress with running spend on the named ingest and report loops, global `--quiet` / `--verbose` output controls, and consistent `--help` examples for recurring workflows. *In progress:* retiring the `_logic.py` monolith into one command group per file, because agent-context-fit is legibility for the dominant reader, so it earns the spine, not just a harden pass. Design: [`docs/design/logic-decomposition.md`](docs/design/logic-decomposition.md), [`docs/design/how-we-build.md`](docs/design/how-we-build.md). [detail](#engineering-legibility--cli-ux-013-onward)
...
- **0.18 Decomposition finish + batch-run visibility** - finish retiring `_logic.py` while batch progress, running cost, and global verbosity controls settle before the CLI contracts freeze. [detail](#0180--decomposition-finish--batch-run-visibility)
...
**Codebase legibility - retiring the `_logic.py` monolith (in progress, ~99%).** The ROADMAP's "one command group per file" target was violated by a single 9k-line module that no agent can hold in context. This earns the feature spine, not a harden pass, because *agent-context-fit is legibility for the dominant reader*. The work is many small, green, pure-relocation PRs, each moving one command group into `distill/commands/<group>.py` (the `register(app)` shape) and lowering a must-only-decrease module-size ratchet. Twenty-two command modules and paper/site/concept/version/display/video/learning/discover support helpers are out, the root callback and concepts app construction now live in owned command modules, and `_logic.py` is down from 9,373 to 113 lines; no production command module imports the monolith, no command sub-apps remain there, and the module-size allowlist is empty. What remains is the `_cli_impl` compatibility bridge surface, including `_resolve_video_channel_name`, topic-change bridge exports, and private compatibility re-exports. The load-bearing hazard - a test that patches a moved symbol by string path passes while testing nothing - is handled by a documented repoint-in-the-same-PR rule and a pre-merge grep gate. Design and the safe procedure: [`docs/design/logic-decomposition.md`](docs/design/logic-decomposition.md); the operating model that promotes it onto the spine: [`docs/design/how-we-build.md`](docs/design/how-we-build.md).
