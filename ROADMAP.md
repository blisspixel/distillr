...
- **0.19 Recurring research profiles + no-metered-cost routing** - profile artifacts, the cost-mode router, the adapter doctor, and profile-run handoff are wired; what remains is the external-dependency graduation (current no-metered support statements, plan-quota auth proof, native usage capture for Grok/Gemini/Antigravity, and `distill eval` route graduation - some of it gated on vendor policy, not effort). [detail](#0190--recurring-research-profiles--no-metered-cost-routing)
