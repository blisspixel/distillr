...
**Run-time verify hook.** *(Status: the deterministic tier shipped in 0.10.0 — numeric/percent/money/year claims grounded against the source receipt on paper and video emits, `_Verify.json` sidecars, `DISTILL_VERIFY=warn|off`. Remaining in 0.10.x: `strict` + the `--verify` CLI flag, the site/tweet/local emit paths, the local entailment-checker tier for named entities and prose claims, and the audit surface below.)*
