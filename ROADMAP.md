...
- **Source breadth and audio capability** — the four remaining adapters (podcasts, GitHub repos, generic audio/video files, Substack/newsletter) on a documented adapter contract, built on the `distill ingest` dispatcher and the Whisper layer the X adapter already proved. [detail](#091--source-breadth-and-audio-capability)
- **Self-maintaining audit** — fold the scattered `distill health` / `doctor --links` / `research_gaps` signals into one `distill audit <topic>` run that emits a report artifact plus an action menu (apply fixes, draft missing notes, ingest gap-fillers). [detail](#092--self-maintaining-audit)
...
- **Manually reviewed dependency upgrades.** `uv.lock` is bumped in a deliberate PR (never auto-resolved in CI), and that PR runs the full CI matrix against the updated lock *before* merge — the exact control that would have caught typer 0.26 as a reviewable red PR instead of a red `main`. Automated bump bots (Dependabot) are deliberately not used in this repo.
...
- Preview-as-primary-flow UX: probe the candidate pool, detect the rerank-score cliff, present "top N excellent / top M including good / everything ≥ threshold" sizing options with per-option spend, then a single typed approval. *(Shipped 0.9.3: on a fresh topic, `discover` now shows the reranked candidates and a size-then-approve menu — Excellent / Including good / Everything worthwhile, each with its source breakdown and its own calibrated spend estimate — and ingests the chosen subset, saving it to the preview cache for exact replay. `--yes` keeps the non-interactive rigor path; `--size` forces the menu on topics that already have artifacts. Built on the pure `build_sizing_options` ladder over the score cliff + rigor thresholds.)*
- Rerank determinism: *(Shipped.)* the discover rerank LLM call runs at `temperature=0` for a reproducible previewed order (0.8.12), and `discover --preview` now caches the exact shortlist under a content-addressed id so `discover --from-preview <id>` replays that set verbatim (0.9.2) — skipping query-generation and the rerank entirely.
- Real cost estimator that reads candidate metadata before the run (arXiv abstract length + page count; yt-dlp duration; site content-length) and calibrates against historical `cost_log.jsonl`. *(Shipped 0.9.1: `estimate_discover_items` scales per-video cost by yt-dlp duration and `load_cost_calibration` derives per-source rates from clean single-source `cost_log.jsonl` runs, falling back to constants when history is thin; the estimate now reports an honest range. Paper PDF page count is deliberately not fetched at discovery — it would need a network call — so papers/sites use flat calibrated rates.)*
- `--rigor strict|balanced|loose` knob across discover/papers/latest. Audit and document the prompt divergence between commands. *(Shipped: `discover` in 0.8.12; extended to `papers` and `latest` in 0.9.4 on per-command calibrated thresholds — discover 0.70/0.50/0.30, papers 0.65/0.45/0.30, latest 0.60/0.40/0.25. The prompt divergence was audited and written up in docs/architecture.md ("Rigor calibration"): discover is a cross-source curation gate, papers/latest are single-source relevance rankers, so their bars sit lower. On papers/latest the default is `off` (no behavior change) and the bar is skipped with a warning under `--no-rerank`.)*
...
- **Gap-driven discovery.** *(Shipped 0.8.11.)* Today `discover` is goal-driven: it starts from a user-written goal and fans out. The `research_gaps(topic)` signal computes the inverse — *what the corpus is thin on* (single-source coverage, too few channels, missing corpus synthesis) plus `next_actions`. `discover --from-gaps --topic <t>` now wires that signal forward: it synthesizes a discovery goal from the gap findings and runs the normal query-generation -> rerank -> preview/approve -> ingest flow. The gap computation was lifted to a shared `distill/pipeline/gaps.py` so the MCP tool and the command share it. This is the corpus-gap-driven complement to goal-driven discovery, and the first half of the self-maintaining loop that 0.9.2's audit completes (the "ingest these" branch of its action menu).

**Synthesis depth.**

- **Two-pass synthesis with a structured intermediate.** *(Shipped 0.9.0, opt-in via `--two-pass`.)* Replace single-pass synthesis with: (1) claim-extraction pass over each per-source insight emitting structured rows into a per-topic `claims.jsonl`; (2) synthesis pass over the claim set that clusters, finds contradictions, and writes the narrative with explicit per-claim citations. The 0.7.2 prompt rewrite raised the quality contract but is still single-pass; the structured intermediate is what makes that contract reliably enforceable. Architecturally this is the same append-only-JSONL + pure-Python-merge pattern 0.8 used for `mentions.jsonl` — the playbook layer validated that the LLM-produces-rows / Python-merges-rows split works in production. Reusing that pattern means concepts can attach evidence to specific claim IDs (instead of whole insight files) once both layers exist. Shipped as `distill/claims/` (`Claim`/`ClaimRole`, `run_claims`, `claims.jsonl`) plus `claim_synthesis_prompt`; wired into `distill resynthesize --two-pass` and the MCP `synthesize` tool's `two_pass` arg. Single-pass remains the default until the 1.0 golden-eval gate validates two-pass quality and flips it; the per-claim fitness cache and metamorphic checks are the remaining 1.0 deferrals.
...
- Synthesis register styles: `--style exec | pop | landscape | disagreements-only` selects emphasis, but every style honors the PhD-level contract shipped in 0.7.2 (cross-paper claims, comparison matrix, named disagreements, shared blind spots). *(Shipped 0.8.10 — `distill resynthesize --style ...` and the MCP `synthesize` `style` arg, via a `STYLE_GUIDANCE` map threaded into the topic and corpus synthesis prompts; default unchanged.)*
- **Anti-AI-slop register guard.** *(Shipped 0.8.8.)* `prompts/shared.py` carried `ANTI_HALLUCINATION_RULES`, `PROVENANCE_RULES`, and a one-line `FORMATTING_RULES` (no em-dashes) — *what is true*, but not *how it reads*. A `REGISTER_RULES` constant grounded in the Wikipedia "signs of AI writing" list (no filler superlatives, no "delve / it's worth noting / in conclusion" scaffolding, no hedge-stacking, no "not only X but also Y") is now threaded into the human-read synthesis/report/brief prompts (topic synthesis, corpus synthesis, report section writer, topic brief). Anti-hallucination keeps the corpus *correct*; this keeps the prose *publishable*. Prompt-layer constant plus its wiring, no new dependency.
