...
- **0.12 Compounding corpus — named scope shipped (0.12.0–0.12.6).** The `distill ask` loop with strict-by-definition promotion, sub-agent MCP summaries (revision-cached), read-only MCP, scheduling recipes, semantic dedup, the prompt-version registry + staleness rollup + re-analysis trigger, estimator accountability. Goal-file watch hook shipped 0.12.7 (persisted goals + catch-up surfacing the exact preview refresh commands). Remaining margins: synthesis stale-flag, per-call MCP spend caps, long-run progress. [detail](#0120--compounding-corpus)
