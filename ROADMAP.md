...
- **Batch progress and running cost.** Status: `papers` and `site-batch` now print phase/item/completed/failed/spend output, with ETA after enough items complete. `site-batch` also isolates seed-level exceptions as `site-ingest` issues and continues. Remaining: carry the same surface through `discover`, `latest`, and `catch-up`; preserve `--json` stdout purity by sending human progress to stderr.
