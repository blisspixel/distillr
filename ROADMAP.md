...
- **0.19.1 cost policy and local routes.** Wire `auto`, `no-metered`, and `paid-ok` through the router, ledger, Ollama, LM Studio, and deterministic fetch/transcript paths. Status: config/router parsing, top-level CLI override, no-metered profile replay commands, and fail-closed route refusal are wired; `no-metered` allows local Ollama/LM Studio and blocks API-billed or ambiguous routes before provider calls. Remaining: profile run integration, ledger rows for zero-dollar usage, and richer blocked-route reporting. Label local routes as sunk-cost compute and record usage even when dollar cost is zero.
...
- **0.19.5 profile run and loop handoff.** Enable approved profile refreshes and next-action JSON so cron, GitHub Actions, Codex, Claude Code, Grok Build, Gemini CLI, Antigravity, or a human operator can steward the profile without scraping console text.
