...
- [~] **Proof artifacts (pulled forward from the 1.0 presentation pass).** The example corpus shipped in 0.9.29 (`examples/library/topics/claim-verification/` — real, labelled, $0.19 of analysis; full-text receipts omitted for arXiv licensing). Remaining: 3–4 screenshots/GIFs (terminal run, dashboard, a synthesis open in Obsidian) and one terminal recording of a `discover --preview -> ingest` run — these need a human at the screen. Demo provenance stays unambiguous: every sample labelled real-or-synthetic.
