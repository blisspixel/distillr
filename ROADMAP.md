...
- **Engineering legibility + CLI-UX (0.13 onward, in progress)** — the run between the named source/trust/compounding milestones and 1.0. *Shipped:* the entailment tier + verify-on-every-synthesis (0.13), agent-grade `--json` / stdout discipline (0.14), `distill update` (0.15), the blocking golden-corpus eval gate (0.16). *In progress:* retiring the `_logic.py` monolith into one command group per file — agent-context-fit is legibility for the dominant reader, so it earns the spine, not just a harden pass — and a human+agent CLI-UX pass (`distill init`, did-you-mean on typos, and a machine-readable `doctor` readiness verdict have landed; live progress + running cost on the batch commands and a `-q`/`-v` verbosity dial come once those commands leave the monolith). Design: [`docs/design/logic-decomposition.md`](docs/design/logic-decomposition.md), [`docs/design/how-we-build.md`](docs/design/how-we-build.md). [detail](#engineering-legibility--cli-ux-013-onward)
- **1.0 Stability commitment + quality bar** — versioned CLI / MCP / library / frontmatter contracts; Pyright-strict and "parse, don't validate" boundaries; the golden-corpus eval gate (+ metamorphic robustness) and verification depth on the deterministic core; branch coverage ≥95%; blocking lint/security CI; the OS support matrix; the presentation pass; and a documented prompt-revision cadence (contracts are stable, prompts are versioned and revised on a schedule). [detail](#100--stability-commitment--quality-bar)
- **Provider breadth + plan-quota compute (committed, post-1.0)** — the eval-gated adapter contract across cloud APIs (xAI, Google, **Anthropic and OpenAI — adapters already in-tree**, AWS Bedrock, Microsoft Foundry) *and* the plan-quota class: route batch analysis through the agent CLIs and subscriptions users already pay for (Claude, Codex, Gemini/Antigravity, Grok, OpenCode, Kiro, local Ollama/LM Studio), with `distill eval` as the gate that decides what is actually usable. [detail](#looking-beyond-10)
...
- [~] **MCP surface consolidation (the §11 just-in-time item, promoted).** Mostly already real: `find_insights` returns ranked `(path, preview, score)` tuples with `read_insight(path, section?)` drill-down (paths-not-payloads is the shipped default), `generate_report` truncates at 5K chars, and 0.9.30 removed the first strict duplicate (`list_contested` folded into `find_concepts(contested_only=True)`; 22 -> 21 tools). Remaining: continue collapsing overlapping action tools toward workflow shapes where it doesn't break the adapter contract's CLI-parity rule, and keep resources/prompts deprioritized (no evidence mainstream clients use either; they cost no tool-schema tokens, so removal is maintenance relief, not context relief).
...
- **Configurable severity** — `--verify warn` (default; surface to console, write anyway), `--verify strict` (refuse the write), `--verify off`. Deterministic verification layered on stochastic output — the Agent SDK "hooks" pattern. **The deterministic numeric presence-check is advisory only: it must NOT hold the `strict` veto by itself.** A presence-check over-refuses correct *derived* numbers ("4.2-point gain" computed from 72.6 vs 68.4 is not literally in the source) and under-catches wrong-metric numbers — so letting it refuse writes blocks good insights on surface absence, the brittle-gate harm the charter warns against. `strict` is held by the model/entailment tier (a real grounding judgment); the regex tier only flags candidates for it to check. ("Is the cited number present?" is structural and fine for *flagging*; "is the claim supported?" is the semantic call that gates.)
...
**Self-maintaining audit.** *(Shipped 0.10.2: `distill audit <topic|all>` composes the verify-sidecar rollup — clean / flagged / never-checked per insight, the panel's "visible trust moment" — with health warnings, contested concepts, broken links, and coverage gaps into a `<topic>_Audit.md` artifact; `--report-only` for unattended runs, interactive action menu otherwise, where only free deterministic actions execute and anything that spends prints its command instead. `health` remains the fast console view. Remaining: wire the scheduled cadence in 0.12 and an MCP `audit` read tool.)* Original spec — the Karpathy-pattern "monthly health check," composed from signals distill already produces:
...
**Codebase legibility — retiring the `_logic.py` monolith (in progress).** The ROADMAP's "one command group per file" target was violated by a single 9k-line module that no agent can hold in context. This earns the feature spine, not a harden pass, because *agent-context-fit is legibility for the dominant reader*. The work is many small, green, pure-relocation PRs, each moving one command group into `distill/commands/<group>.py` (the `register(app)` shape) and lowering a must-only-decrease module-size ratchet. The load-bearing hazard — a test that patches a moved symbol by string path passes while testing nothing — is handled by a documented repoint-in-the-same-PR rule and a pre-merge grep gate. Design and the safe procedure: [`docs/design/logic-decomposition.md`](docs/design/logic-decomposition.md); the operating model that promotes it onto the spine: [`docs/design/how-we-build.md`](docs/design/how-we-build.md).

**Human + agent CLI-UX.** Aligned to current CLI best practice (clig.dev-era): a guided, no-TTY-safe `distill init` (creates `.env`, validates the key, installs the browser, ends on a ready/not-ready verdict), did-you-mean on mistyped commands, and a machine-readable `doctor` readiness verdict have landed. The flagship still ahead — live progress with a running cost readout on the batch commands (`papers`, `discover`, `latest`, `catch-up`, `site-batch`), plus a `-q`/`-v` verbosity dial and consistent `--help` examples — is **gated on the decomposition**: those commands' loops live inside `_logic.py` today, so they leave the monolith first, then gain the UX in their new home.
...
- **Golden corpus eval gate — model-judged, NOT regex-scored.** A frozen ~20-paper reference corpus ships with hand-checked golden insights. CI runs the analysis pipeline against it and gates on a **model judgment of the candidate output against the golden and the source** — the `judge_faithfulness` / coverage check (model proposes per-criterion verdicts; Python aggregates and thresholds), the same shape used for the migration gate. **Per the charter ([`docs/design/agentic-balance.md`](docs/design/agentic-balance.md)) the gate must NOT score "per-section agreement" with the deterministic composite** (`scoring.py`: capitalization-regex coverage + word-count depth + substring `##`). That composite is a brittle proxy — it fails good paraphrase and passes keyword-stuffing — and as a *blocking* gate it would force every prompt change to be shaped around gaming a regex, making the product worse. **Rework owed before 1.0:** 0.16.0 shipped the blocking gate using exactly that composite as the per-dimension floor; it must move to the model-judged form. The deterministic composite survives only as a cheap *discrimination tripwire* (a deliberately-degraded output must still fail) and an advisory diagnostic, never as the quality bar. Still ahead: a golden for the concept-playbook pipeline, the ~20-paper scale-up, an `eval_models` MCP tool. Catches the regression class coverage/type/lint miss (prompt drift, model swaps, silent extraction degradation).
