...
- **0.19.1 cost policy and local routes.** Wire `auto`, `no-metered`, and `paid-ok` through the router, ledger, Ollama, LM Studio, and deterministic fetch/transcript paths. Status: config/router parsing and fail-closed route refusal are wired; `no-metered` allows local Ollama/LM Studio and blocks API-billed or ambiguous routes before provider calls. Remaining: CLI `--cost-mode` overrides, profile run integration, ledger rows for zero-dollar usage, and richer blocked-route reporting. Label local routes as sunk-cost compute and record usage even when dollar cost is zero.
