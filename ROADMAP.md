...
- **Engineering legibility + CLI-UX (0.13 onward, in progress)** - the run between the named source/trust/compounding milestones and 1.0. *Shipped:* the entailment tier + verify-on-every-synthesis (0.13), agent-grade `--json` / stdout discipline (0.14), `distill update` (0.15), the blocking golden-corpus eval gate (0.16), and per-item progress with running spend on the named ingest loops. *In progress:* retiring the `_logic.py` monolith into one command group per file, because agent-context-fit is legibility for the dominant reader, so it earns the spine, not just a harden pass; and the remaining human+agent CLI-UX pass (`distill init`, did-you-mean on typos, a machine-readable `doctor` readiness verdict, and named ingest-loop progress have landed; report progress and a `-q`/`-v` verbosity dial remain). Design: [`docs/design/logic-decomposition.md`](docs/design/logic-decomposition.md), [`docs/design/how-we-build.md`](docs/design/how-we-build.md). [detail](#engineering-legibility--cli-ux-013-onward)
...
**Human + agent CLI-UX.** Aligned to current CLI best practice (clig.dev-era): a guided, no-TTY-safe `distill init` (creates `.env`, validates the key, installs the browser, ends on a ready/not-ready verdict), did-you-mean on mistyped commands, and a machine-readable `doctor` readiness verdict have landed. The flagship still ahead is live progress with a running cost readout on the batch commands (`papers`, `discover`, `latest`, `catch-up`, `site-batch`), plus a `-q`/`-v` verbosity dial and consistent `--help` examples. Status: `papers`, `site-batch`, `latest`, and `catch-up` now print phase or item progress, completed counts, failed counts, spend, and ETA when available; `discover` still needs the same surface.
...
- **Batch progress and running cost.** Status: `papers` and `site-batch` now print phase/item/completed/failed/spend output, with ETA after enough items complete. Video-backed loops used by `latest` and `catch-up` now print persistent per-video completed/failed/spend progress after each item and include spend in live phase labels. `site-batch` also isolates seed-level exceptions as `site-ingest` issues and continues. Remaining: carry the same surface through `discover`; preserve `--json` stdout purity by sending human progress to stderr.
