...
**Codebase legibility - retiring the `_logic.py` monolith (in progress, ~95%).** The ROADMAP's "one command group per file" target was violated by a single 9k-line module that no agent can hold in context. This earns the feature spine, not a harden pass, because *agent-context-fit is legibility for the dominant reader*. The work is many small, green, pure-relocation PRs, each moving one command group into `distill/commands/<group>.py` (the `register(app)` shape) and lowering a must-only-decrease module-size ratchet. Twenty-two command modules and paper/site/concept/version/display/video/learning-selection/learning-flow support helpers are out, and `_logic.py` is down from 9,373 to 470 lines; no command sub-apps remain in the monolith and the module-size allowlist is empty. What remains is the root home-screen callback, discover/process helper body, topic-change bridge exports, and compatibility re-exports. The load-bearing hazard - a test that patches a moved symbol by string path passes while testing nothing - is handled by a documented repoint-in-the-same-PR rule and a pre-merge grep gate. Design and the safe procedure: [`docs/design/logic-decomposition.md`](docs/design/logic-decomposition.md); the operating model that promotes it onto the spine: [`docs/design/how-we-build.md`](docs/design/how-we-build.md).
