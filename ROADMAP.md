...
Shipped: **0.1 through 0.17**, re-validated against the code, the test suite, and a live model run on 2026-06-20 (see [Validation](#validation-2026-06-20)). Per-release detail is the changelog's job, not the roadmap's: [`docs/CHANGELOG.md`](docs/CHANGELOG.md). Newest-first headlines:
...
**Remaining to 1.0.** The feature spine is essentially complete; the distance left is mostly the quality bar, not new surface. Forward milestones, detail below:

- **0.18 Batch-run visibility** - `_logic.py` is retired and batch progress / running cost / verbosity controls largely shipped; what is left is the last verbosity-dial polish and quality ratchets before the CLI contracts freeze. [detail](#0180--decomposition-finish--batch-run-visibility)
- **0.19 Recurring research profiles + no-metered-cost routing** - profile artifacts, the cost-mode router, the adapter doctor, profile-run handoff, and native usage capture for the planned CLI adapters are wired; what remains is the external-dependency graduation (current no-metered support statements, plan-quota auth proof, native schema enforcement where the CLI supports it, and `distill eval` route graduation - some of it gated on vendor policy, not effort). [detail](#0190--recurring-research-profiles--no-metered-cost-routing)
- **1.0 Stability commitment + quality bar** - the real remaining distance: versioned and frozen CLI / MCP / library / frontmatter / OKF / next-action / profile contracts; Pyright-strict and "parse, don't validate" at every boundary; verification depth on the deterministic core; branch coverage 80 -> >=95%; and the presentation pass. [detail](#100--stability-commitment--quality-bar)
- **Provider breadth + plan-quota compute (committed, post-1.0)** - the eval-gated adapter contract across cloud APIs (xAI, Google, Anthropic and OpenAI in-tree, AWS Bedrock, Microsoft Foundry) and the plan-quota class. [detail](#looking-beyond-10)
...
- **0.19.2 adapter doctor.** Add read-only preflight checks for Codex CLI, Claude Code, Grok Build, Gemini CLI, and Antigravity: installed version, auth mode, blocked API-key state, headless support, machine-readable output support, and support-statement version. Status: `distill doctor --adapters` reports binary presence, version/help probes, required structured-output flags, API-key environment blockers, local config API-key markers, selected JSON auth-command markers, route class, support-statement status, structured support details, and strict `adapter-workload.v1`, `adapter-native-usage.v1`, and `adapter-result.v1` scratch contracts. Support details record checked date, source URLs, required evidence, notes, and whether the statement is current for no-metered routing. The manifest boundary rejects missing usage signals, unsafe scratch paths, missing declared files, unexpected new scratch files, no-metered results with metered auth or API-key blockers, and quota or rate-limit stops without structured `quota_stop` metadata. A scratch-only exact-argv runner primitive exists with shell disabled, timeout handling, API-key env stripping, manifest loading, and scratch write checks. A workload runner now invokes optional post-process capture hooks before verifying manifest reads, writes, and cost mode against a checked workload package. A native result writer turns captured CLI output plus explicit native usage metadata or a validated native usage file into validated scratch manifests. Codex, Claude, Grok, Gemini CLI, and Antigravity capture writers can convert captured CLI JSON into that scratch manifest shape, and a generic stdout capture writer can write `result.txt` before manifest validation when a real native usage file exists. A ledger helper converts verified manifests into included-plan usage rows and metadata. Blocked Codex, Claude, Grok, Gemini, and Antigravity read-only command planners record future argv shapes plus staged prompt, schema, result capture, and native usage capture metadata, and Claude schema paths can be inlined from scratch JSON schema files. Gemini and Antigravity fail closed on `GOOGLE_API_KEY` as well as `GEMINI_API_KEY`. Remaining: current official no-metered support statements, installed-session auth proof where no command or config proof exists, native schema enforcement where the CLI supports it, and eval graduation. Report Copilot separately as a credit-metered CLI candidate under explicit paid policy.
- **0.19.3 cross-route quality eval.** Add fixture comparisons for local sunk-cost routes, plan-quota CLI routes, and metered API routes. Use LLM-as-judge scoring over receipts plus cost-per-accepted-change accounting so fan-out routing is based on output quality and retained work, not just availability. Status: `distill.eval.graduation` now exposes a pure graduation decision that combines adapter doctor readiness with model-judged eval evidence and fails closed before any adapter route can enter no-metered routing. `distill.eval.route_pool` consumes graduation decisions for pure route-pool admission, prefers local routes, admits included-plan adapters only after proof, and keeps credit-metered routes behind `paid-ok`. `distill.eval.route_availability` adds a quotabot-inspired pure availability contract for local service status, installed local models, portable quota windows, stale signals, and manifest quota stops, so live orchestration can require fresh proof and evict exhausted routes without provider scraping inside the selector. Portable availability snapshots are account-neutral and reject identity-bearing quota metadata.
...
- **0.19.6 route orchestration strategies.** Once read-only adapters (0.19.4) and cross-route eval (0.19.3) exist, add a strategy layer over the adapter runner that uses several validated routes *together* rather than picking one: single, ensemble best-of-N (fan out the same task, cross-family judge picks/synthesizes the winner), maker-checker (a different-family route verifies and refines against the receipts), and bounded critic-refine. `distill eval` scores `(workload, strategy)` on cost per accepted change and recommends the cheapest clearing strategy per workload, pool-aware. The verifier stays receipt-grounded and model-judged, judges never grade their own family, selection uses the reliable modes (a coarse faithfulness veto plus pairwise ranking, never an absolute quality score), and a `quota_stop` evicts a route from the live pool. Buildable and testable against local plus mock routes before any plan-quota vendor statement is current. Design: [`docs/design/route-orchestration.md`](docs/design/route-orchestration.md).
...
- **Provider prompt and context caching research spike.** Before enabling any provider-side prompt caching knobs, research the active provider semantics for Anthropic, OpenAI, Gemini, Bedrock, Foundry, and xAI. The gate is provider-specific economics, not a generic "cache on" flag: Anthropic cache writes cost more than base input and 1-hour TTL costs more again, OpenAI caching is automatic with no separate write fee but still needs hit-rate telemetry, and Gemini explicit context caching can add storage-time charges. Any implementation must be opt-in per provider, record cached token and storage metrics in the ledger, avoid pre-warming unless projected savings are positive, set explicit TTL or retention bounds, stop background cache refreshes when the command exits, and never claim no-metered savings unless the route proof and usage ledger show it.
