...
- **0.12 Compounding corpus — named scope shipped (0.12.0–0.12.6).** The `distill ask` loop with strict-by-definition promotion, sub-agent MCP summaries (revision-cached), read-only MCP, scheduling recipes, semantic dedup, the prompt-version registry + staleness rollup + re-analysis trigger, estimator accountability. Goal-file watch hook shipped 0.12.7 (persisted goals + catch-up surfacing the exact preview refresh commands); synthesis stale-flag shipped 0.12.8 (source-relative freshness in audit + dashboard + orientation files, planned from a full review of the 53-topic dev library, which also caught and fixed paper-only topics never refreshing orientation files). MCP per-call spend caps + ingest-domain allowlist shipped 0.12.9; per-item failure isolation + the convergent resume hint shipped 0.12.10 — **every named 0.12 margin is now closed.** Remaining before 1.0: the dev-library review items in the milestone detail, the 0.11 YouTube-resilience margin, and the parallel-track entailment tier. [detail](#0120--compounding-corpus)
