...
- **0.10 Verified corpus — core shipped (0.10.0–0.10.2).** The deterministic claim-grounding hook on every emit path + `distill audit`. Remaining 0.10.x: the **local entailment-checker tier** (HHEM-class, for named entities and prose claims — the gap the deterministic tier deliberately leaves) and verify on synthesis emits. The entailment tier is parallel-track: it strengthens the gate but does not block 0.11, because the deterministic gate already stands. [detail](#0100--verified-corpus-run-time-verify--self-maintaining-audit)
- **0.11 Source breadth and audio capability — shipped (0.11.0–0.11.2).** All five adapters live (GitHub repos, podcasts RSS-first, generic media files, newsletters, plus the 0.9 X adapter) on the documented contract, every one behind the verify gate. Remaining 0.11.x margin: YouTube-path resilience. [detail](#0110--source-breadth-and-audio-capability)
- **0.12 Compounding corpus — named scope shipped (0.12.0–0.12.6).** The `distill ask` loop with strict-by-definition promotion, sub-agent MCP summaries (revision-cached), read-only MCP, scheduling recipes, semantic dedup, the prompt-version registry + staleness rollup + re-analysis trigger, estimator accountability. Goal-file watch hook shipped 0.12.7 (persisted goals + catch-up surfacing the exact preview refresh commands); synthesis stale-flag shipped 0.12.8 (source-relative freshness in audit + dashboard + orientation files, planned from a full review of the 53-topic dev library, which also caught and fixed paper-only topics never refreshing orientation files). Remaining margins: per-call MCP spend caps, long-run progress, and the dev-library review items in the milestone detail. [detail](#0120--compounding-corpus)
