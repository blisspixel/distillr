...
- **MCP write-side gating** (from the June 2026 panel review, enterprise-architect finding): the spend-and-ingest MCP tools (`papers`, `discover`, `site_batch`, `process_video_url`) are callable by any connected agent with no per-call spend cap, no ingest-domain allowlist, and no read-only mode — a confused or injected agent can burn budget or poison the corpus by tool call. Add `DISTILL_MCP_READ_ONLY` (serve only the read surface) and route the write tools through the same budget guardrails as the CLI; document "read-only MCP, CLI ingest by a named operator" as the recommended agent-facing posture until then.
