...
- **0.19.3 cross-route quality eval.** Add fixture comparisons for local sunk-cost routes, plan-quota CLI routes, and metered API routes. Use LLM-as-judge scoring over receipts plus cost-per-accepted-change accounting so fan-out routing is based on output quality and retained work, not just availability. Status: `distill.eval.graduation` now exposes a pure graduation decision that combines adapter doctor readiness with model-judged eval evidence and fails closed before any adapter route can enter no-metered routing. `distill.eval.route_pool` consumes graduation decisions for pure route-pool admission, prefers local routes, admits included-plan adapters only after proof, and keeps credit-metered routes behind `paid-ok`.
