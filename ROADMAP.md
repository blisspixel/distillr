...
- **0.10 Verified corpus — shipped (0.10.0–0.10.2 core; entailment tier shipped 0.13.0).** The deterministic claim-grounding hook on every emit path + `distill audit`, and now the **local entailment tier** (HHEM-2.1-Open behind `distillr[entailment]`, prose claims scored against receipts, sidecar schema v2 additive, strict refuses on prose flags too) plus verify on the paper-synthesis emit (corpus/topic synthesis receipts follow 0.13.1). Design: [`docs/design/entailment-tier.md`](docs/design/entailment-tier.md). [detail](#0100--verified-corpus-run-time-verify--self-maintaining-audit)
