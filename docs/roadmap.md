...
- [~] Projected next-run cost by workflow, not just historical spend.
  ...
  exceeds the configured cap. Remaining: extend the same pre-run guard to
...
- [~] Cost anomaly detection and budget guardrails per topic or workflow so
  ...
  projected spend exceeds the configured workflow cap. Remaining: broader
