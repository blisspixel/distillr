...
- [ ] **Local and plan-quota routing.** Prefer deterministic work, local model servers, and explicitly configured plan-quota CLIs such as Codex CLI, Claude Code, and Grok Build when their adapter support statement is current and they pass `distill eval`.
