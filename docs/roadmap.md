...
- [~] **Billing preflights.** Fail closed when route cost is ambiguous. Adapter doctor reports API-key environment blockers and local config API-key markers for candidate plan-quota CLIs, including `ANTHROPIC_API_KEY` for Claude Code and `GOOGLE_API_KEY` for Gemini-family CLIs, and the workload, usage, and manifest boundaries reject unsafe scratch paths, no-metered results that carry metered auth, API-key blockers, missing usage signals, missing declared files, unexpected new scratch files, and workload result drift. Remaining: official installed-session auth proof, Antigravity command template, native usage collection for non-Codex CLI outputs, and eval graduation.
