...
- [~] Projected next-run cost by workflow, not just historical spend.
  ...
  `ask` cap, while no-coverage asks remain free. `distill site-batch` now
  refuses before model preflight when the resolved max-page, synthesis, and
  optional report-tail projection exceeds the configured `site-batch` cap,
  while preview and scrape-only paths remain free. `distill video`,
  ...
  exceeds the configured cap. Direct `distill synthesize`, `distill topic
...
- [~] Cost anomaly detection and budget guardrails per topic or workflow so
  ...
  `report`, `research-brief`, `ask`, `site-batch`, `video`, `channel`,
  `catch-up`, `reanalyze`, and `resynthesize` workflows now stop before the
  estimated work starts when projected spend exceeds the configured workflow
  cap.
