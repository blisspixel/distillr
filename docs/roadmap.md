...
- [~] **Branch coverage ratchet** (0.8.3 -> 1.0) - `[tool.coverage.run] branch = true`, `--cov-fail-under` set to the measured branch baseline (floor 79) and ratcheted up-only toward the 1.0 flat >=95% gate. Current measured full-suite coverage is 89.79% after startup compatibility coverage for `_app.py`, `_version.py`, and the legacy topic-change shim; the enforced floor remains 89 until the suite clears the next whole-point ratchet.
