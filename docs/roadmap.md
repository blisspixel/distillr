...
- [~] Projected next-run cost by workflow, not just historical spend.
  ...
  workflow cap. Remaining: extend the same pre-run guard to additional direct
  commands once they expose credible estimates before their billable phase.
...
- [~] Cost anomaly detection and budget guardrails per topic or workflow so
  ...
  `report`, and `research-brief` workflows now stop before the estimated work
  starts when projected spend exceeds the configured workflow cap. Remaining:
  broader pre-run projected spend checks for direct one-off CLI commands before
  their billable phase where a reliable estimate exists.
