...
- [~] **Branch coverage ratchet** (0.8.3 -> 1.0) - `[tool.coverage.run] branch = true`, `--cov-fail-under` set to the measured branch baseline (floor 79) and ratcheted up-only toward the 1.0 flat >=95% gate. Current measured full-suite coverage is 94.15% after router provider-cache boundary coverage, AgentProvider task-file path-boundary coverage, Gemini provider boundary coverage, eval judge parser/ledger coverage, eval model-provider inference coverage, local model-recommendation boundary coverage, local hardware-doctor boundary coverage, and CLI provider-status exit-code coverage, CLI entrypoint provider-error coverage, topic-watch management branch coverage, maintain-command branch coverage, dashboard branch coverage, ranking branch coverage, gap-analysis branch coverage, recurring-profile health branch coverage, recurring-profile preview coverage, recurring-profile run coverage, search branch coverage, site-analysis branch coverage, video-analysis branch coverage, newsletter-analysis branch coverage, podcast-analysis branch coverage, podcast-feed boundary coverage, X media boundary coverage, post-ingest concept helper coverage, topic-resolution helper coverage, OKF command wrapper coverage, ask command wrapper coverage, root command wrapper coverage, discover-ingest helper coverage, site-batch helper coverage, discover command wrapper coverage, topic command helper coverage, maintenance fallback cleanup, discover-flow helper coverage, ingest command coverage, MCP papers tool coverage, MCP discover tool coverage, MCP site-batch tool coverage, MCP cost-history tool coverage, MCP OKF tool coverage, the 0.19.10 production no-op fallback cleanup, projected workflow budget refusal coverage, Sonnet 5 provider compatibility coverage, ask answer-artifact citation write-guard coverage, ask projected workflow budget refusal coverage, site and site-batch projected workflow budget refusal coverage, paper workflow projected budget refusal coverage, corpus projected workflow budget refusal coverage, concept canonicalization idempotence regression coverage, MCP ask citation-refusal status coverage, Deep Research report citation-refusal coverage, Deep Research workflow projected budget refusal coverage, video/reprocess workflow projected budget refusal coverage, synthesis workflow projected budget refusal coverage, and recent deterministic-core verification-depth (migration, cost-anomaly, citation-export, and research-profile paths), the corpus-quality-trend and snapshot surface, and the Ollama context-window fallback; the enforced floor is now 93.
