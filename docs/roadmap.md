...
- [~] Live mixed-source run progress so long `discover` / `report` / site-heavy jobs show current phase, current item, completed/failed counts, and where time is going without making the user inspect the filesystem. Status: `papers` and `site-batch` now show phase/item/completed/failed/spend output, with ETA when enough items have completed. Video-backed loops used by `latest` and `catch-up` now print persistent per-video completed/failed/spend progress after each item and include spend in live phase labels. Remaining: extend the same surface to `discover`.
