...
- [~] **Finish the `_logic.py` decomposition, with removal criteria** (in progress, ~95%). `distill/commands/_logic.py` was the center of gravity - one ~9k-line module holding the full implementation of every CLI command, with `_cli_impl` as a compatibility alias. External code QA (2026-06-11) called it the largest maintainability risk visible from the code surface, and it is the original 0.7 code-health item. Twenty-two command modules and paper/site/concept/version/display/video/learning-selection/learning-flow support helpers are now out, and `_logic.py` is down from 9,373 to 470 lines; no command sub-apps remain inside and the module-size allowlist is empty, only the root home-screen callback, discover/process helper body, topic-change bridge exports, and compatibility re-exports remain. Target: command modules that call workflow services; the shims are acceptable only during migration, so the removal criteria are defined up front (every command in its own `commands/<verb>.py`, `_logic.py` deleted, import-linter contract keeping command modules from importing each other) and ratcheted toward one command group per pass. Per-slice plan: [`docs/design/logic-decomposition.md`](design/logic-decomposition.md).
