...
- [~] **Branch coverage ratchet** (0.8.3 -> 1.0) - `[tool.coverage.run] branch = true`, `--cov-fail-under` set to the measured branch baseline (floor 79) and ratcheted up-only toward the 1.0 flat >=95% gate. Current measured full-suite coverage is 90.14% after CLI provider-status exit-code coverage, CLI entrypoint provider-error coverage, and topic-watch management branch coverage; the enforced floor is now 90.
