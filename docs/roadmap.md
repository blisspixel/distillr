...
- [~] **Profile run.** `distill profile run <name>` now plans approved preview commands, requires `--yes` before execution, runs the existing `distill ...` ingest and analysis paths, captures per-command exits, and writes resume state under `.distill/profiles/<profile>/run_state.json`. Exact feed items and YouTube videos complete once; standing seeds stay repeatable. Remaining: audit-visible profile health and profile next-action rows.
- [~] **No-metered-cost mode.** Add `DISTILL_COST_MODE=auto|no-metered|paid-ok` plus `--cost-mode`. Core config/router parsing, top-level CLI override, fail-closed refusal, no-metered profile replay commands, profile-run execution of those commands, and zero-dollar route/provider ledger rows are wired: `no-metered` allows local Ollama/LM Studio and blocks API-billed or ambiguous routes before provider calls. Remaining: richer blocked-route reporting.
- [ ] **Local and plan-quota routing.** Prefer deterministic work, local model servers, and explicitly configured plan-quota CLIs such as Codex CLI, Claude Code, Grok Build, Gemini CLI, and Antigravity when their adapter support statement is current and they pass `distill eval`. GitHub Copilot CLI can be supported as an explicit credit-metered route, but it is not a no-metered default.
- [ ] **Billing preflights.** Fail closed when route cost is ambiguous. Claude Code subscription routes must block if `ANTHROPIC_API_KEY` would force API billing.
- [~] **Complete usage ledger.** Cost-log rows now record provider breakdowns, route-class breakdowns, no-metered call counts, local transcription counts, and zero-dollar `profile-run` orchestration rows. Remaining: native usage signals and quota/rate-limit stops for future plan-quota adapters.
- [ ] **Loop handoff.** Emit profile-related next-action rows compatible with the 0.17 schema so external loops can steward recurring topics without scraping console output.
