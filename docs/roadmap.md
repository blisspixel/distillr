...
- [ ] **Full Pyright-strict ratchet** (1.0) - complete the per-package climb 0.8.3 begins (`distill/llm/` already strict-blocking); no `# type: ignore` without an inline reason. Current strict surface now includes the model-routing ranking boundary in `distill.pipeline.ranking`, the loop-readiness gap analysis boundary in `distill.pipeline.gaps`, the full `distill.pipeline.analysis` package across website, video, paper, newsletter, local-file, media, podcast, repository, and X ingestion boundaries, the full `distill.pipeline.synthesis` package for channel, topic, corpus, and two-pass claim synthesis, recurring profile preview/run orchestration, run-summary reporting in `distill.pipeline.summary`, the report interactions boundary in `distill.pipeline.report._interactions`, the report package export boundary in `distill.pipeline.report`, the single-call report synthesis boundary in `distill.pipeline.report.synthesize`, the lightweight report briefing boundary in `distill.pipeline.report.briefing`, the Gemini Deep Research report boundary in `distill.pipeline.report.deep_research`, the Gemini File Search store and corpus boundary in `distill.pipeline.report.file_search`, and the multi-topic research briefing boundary in `distill.pipeline.report.brief`.
- [ ] **Parse, don't validate - strict domain types at every boundary** (1.0) - parse every external input (MCP tool arguments, frontmatter, adapter/local-file ingest, LLM structured outputs) once at the boundary into a rich domain type (`strict=True, extra='forbid'` Pydantic model, `NewType`, or frozen dataclass); core logic never sees raw primitives. The rerank response parsers now turn LLM JSON rows into typed object dictionaries before ranking logic reads ids or scores, gap analysis now parses metadata plus inventory and summary payloads into typed structures before audit, MCP, or discovery consumes them, and File Search corpus assembly for reports and research briefings parses video, site, and paper metadata before titles or URLs enter uploaded documents.
