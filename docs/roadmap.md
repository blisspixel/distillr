...
- [~] **No-metered-cost mode.** Add `DISTILL_COST_MODE=auto|no-metered|paid-ok` plus `--cost-mode`. Core config/router parsing, top-level CLI override, fail-closed refusal, and no-metered profile replay commands are wired: `no-metered` allows local Ollama/LM Studio and blocks API-billed or ambiguous routes before provider calls. Remaining: profile run integration, ledger rows for zero-dollar usage, and richer blocked-route reporting.
