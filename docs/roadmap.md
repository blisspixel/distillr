...
- [ ] **Full Pyright-strict ratchet** (1.0) - complete the per-package climb 0.8.3 begins (`distill/llm/` already strict-blocking); no `# type: ignore` without an inline reason. Current strict surface now includes the model-routing ranking boundary in `distill.pipeline.ranking`, the loop-readiness gap analysis boundary in `distill.pipeline.gaps`, the full `distill.pipeline.analysis` package across website, video, paper, newsletter, local-file, media, podcast, repository, and X ingestion boundaries, the full `distill.pipeline.synthesis` package for channel, topic, corpus, and two-pass claim synthesis, recurring profile preview/run orchestration, run-summary reporting in `distill.pipeline.summary`, the report interactions boundary in `distill.pipeline.report._interactions`, the single-call report synthesis boundary in `distill.pipeline.report.synthesize`, the lightweight report briefing boundary in `distill.pipeline.report.briefing`, the Gemini Deep Research report boundary in `distill.pipeline.report.deep_research`, the Gemini File Search store and corpus boundary in `distill.pipeline.report.file_search`, and the multi-topic research briefing boundary in `distill.pipeline.report.brief`.
