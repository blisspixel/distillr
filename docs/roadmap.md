...
- [~] Projected next-run cost by workflow, not just historical spend.
  ...
  workflow cap. `distill video`, `distill channel`, `distill catch-up`,
...
- [~] Cost anomaly detection and budget guardrails per topic or workflow so
  ...
  `report`, `research-brief`, `video`, `channel`, `catch-up`, `reanalyze`, and
  `resynthesize` workflows now stop before the estimated work starts when
  projected spend exceeds the configured workflow cap. Synthesis-call workflows
