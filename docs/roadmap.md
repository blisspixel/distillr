...
- [~] Projected next-run cost by workflow, not just historical spend.
  ...
  `ask` cap, while no-coverage asks remain free. `distill video`,
...
- [~] Cost anomaly detection and budget guardrails per topic or workflow so
  ...
  `report`, `research-brief`, `ask`, `video`, `channel`, `catch-up`,
  `reanalyze`, and `resynthesize` workflows now stop before the estimated work
  starts when projected spend exceeds the configured workflow cap.
