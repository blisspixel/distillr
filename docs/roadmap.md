...
- [~] **No-metered-cost mode.** Add `DISTILL_COST_MODE=auto|no-metered|paid-ok` plus `--cost-mode`. Core config/router parsing, top-level CLI override, fail-closed refusal, no-metered profile replay commands, and profile-run execution of those commands are wired: `no-metered` allows local Ollama/LM Studio and blocks API-billed or ambiguous routes before provider calls. Remaining: ledger rows for zero-dollar usage and richer blocked-route reporting.
...
- [ ] **Complete usage ledger.** Record provider, route, model or native usage signal, approximate tokens when available, elapsed time, and quota/rate-limit stops even when dollar cost is zero.
