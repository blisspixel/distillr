...
- [~] Mixed exact-page and shallow-crawl workflows that are easier to understand and safer by default. `distill site-batch --preview` now shows the resolved exact-page versus shallow-crawl plan before any model check, crawl, or write, global `--json` returns that plan as loop-readable rows, MCP `site_batch` honors JSON seed modes, and JSON URL objects or collections can declare `mode: "exact-page"` or `mode: "shallow-crawl"`. Remaining: broader site workflow polish and profile-level branch defaults.
