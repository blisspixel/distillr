...
- [x] **Branch coverage ratchet** (0.8.3 -> 1.0) - `[tool.coverage.run] branch = true`; the enforced full-suite floor is now 95%, and the 0.19.30 release gate measures 95.04% branch coverage across 3,970 passing tests. The ratchet reached the 1.0 target through focused boundary coverage across configuration, providers, commands, MCP, ingestion, library state, deterministic verification, and the contract-snapshot surface.
