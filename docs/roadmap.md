...
- [~] **Profile run.** `distill profile run <name>` now plans approved preview commands, requires `--yes` before execution, runs the existing `distill ...` ingest and analysis paths, captures per-command exits, emits loop-readable next actions, and writes resume state under `.distill/profiles/<profile>/run_state.json`. Exact feed items and YouTube videos complete once; standing seeds stay repeatable. Remaining: audit-visible profile health.
