...
- Raised `distill init` branch coverage to 99% with hermetic tests for browser
  discovery and installation, canonical doctor validation, local Ollama and LM
  Studio probes, interactive key persistence, readiness, and refusal guidance.
...
- Made `distill report` and `distill research-brief` refuse before the Gemini
  Deep Research call when their projected spend exceeds the configured
  workflow cap.
...
- Refused single-call synthesis writes that contain unresolved numbered report
  citations such as `[cite: 1]`, so unresolvable handles do not land in output
  artifacts.
...
- Added focused discover-ingest helper coverage for skipped null candidates,
  ...
  coverage gate now reports 92.18%; the enforced floor remains 92.
...
- Added focused newsletter-analysis branch coverage for empty feed routing,
  ...
  91.87%; the enforced floor remains 91.
...
- Added focused recurring-profile health branch coverage for clean recent
  ...
  The full coverage gate now reports 91.41%; the enforced floor remains 91.
...
- Promoted the top-level Typer command group to Pyright strict mode, added a
  ...
  full run for that ratchet reported 89.79%.
