...
| Plan-quota CLI routes, such as Codex CLI, Claude Code, Grok Build, and Gemini/Antigravity | Planned | Included quota only if proven | Not live providers yet. Adapter doctor preflights, structured support-statement details, local config auth-marker scanning, strict `adapter-workload.v1` input packages, strict `adapter-result.v1` manifest checks with quota-stop metadata, a scratch-only runner primitive, a checked workload runner, a native result writer, a manifest-to-ledger helper, and a blocked Codex read-only command planner exist. No plan-quota support statement is current for no-metered routing yet, and routes still need included-plan auth proof, adapter-specific capture wiring, more adapter command templates, native usage collection, and `distill eval` evidence. |
