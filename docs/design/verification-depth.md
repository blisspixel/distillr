...
4. **It is greenfield and bounded, not grind.** The strict ratchet's remaining
   ...
   `pipeline/` package is now strict-clean, and the full MCP package is now
   strict-clean across the server registration surface, optional progress
   ...
   command, eval command, self-update command, topic/channel resolver, and
   topic-watch helper API are now strict-clean, with JSON envelope parsing,
   discover-ingest, adaptive-ingest, site-batch, trusted-site discovery
   injection, eval preflight, and topic-watch ranking parsed through typed
   public seams. The
   ...
   python-docx renderer surfaces are warning-clean. The advisory
...
Status (2026-06-28): executable contracts now cover the merge interval/source
...
frontmatter-to-rollup row shape. Library frontmatter emit/merge helpers now
assert parseable fenced blocks and key round-trips, wiki-link parsing asserts
render/parse round-trips, and link check/fix helpers assert internally
consistent result shapes. Generated-test wiring now covers `parse_note_fields`,
`canonicalize`, the path sanitizers, explicit `ConceptMention` dataclass
strategies for grouping, threshold, and merge contracts, plus explicit
frontmatter and wiki-link parse strategies.
