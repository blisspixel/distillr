...
4. **No statistics / no calibration (HIGH) — OPEN.** N=3 fixtures per workload (`fixtures.py:217-219`) drive a go/no-go via a bare `>=`; win-rate is a mean of 2 deterministic comparisons; judge accuracy is never calibrated. **Fix:** paired bootstrap CI on (candidate − anchor) per fixture, require the lower bound to clear the bar; min-N before "high" confidence; sample the judge k≥3 at temp>0 for a self-consistency band; **calibrate the judge against the golden/degraded pairs already in `golden.py`** (Cohen's kappa, *derive* the win-rate floor instead of hard-coding 0.45).
...
The two CRITICAL items are fixed (2026-06-13) and the circularity item #3 is fixed (2026-06-14, via the faithfulness veto floor above). **#4 (statistics / calibration) is the remaining eval-gate work**, still sequenced before the P1-P4 ranking-fallback remediation since the eval gate is the arbiter those are tested against. The lesson from #3 is now a standing rule: *the fix for a brittle rule is not always "add a model" — it is "add the model in the mode the evidence says it is reliable."* Pairwise for ranking, coarse absolute for grounding; never a fine-grained absolute "quality score," which is just a different brittle proxy.
