...
3. **Reference-against-incumbent circularity (HIGH) — OPEN.** Even with the gate fixed, the judge is still pairwise-vs-the-anchor's-output, so a *different-but-better* candidate can be penalized for diverging from the incumbent's framing. **Fix:** add absolute source-anchored faithfulness/coverage scoring (graded per output against the SOURCE), use pairwise-vs-anchor only as a secondary tiebreaker, and drop the deterministic heuristic prior from the *gating* judge prompt so it stays independent of the brittle signal.
...
The two CRITICAL items are fixed (2026-06-13); #3 and #4 remain and are the next eval-gate work, sequenced before the P1-P4 ranking-fallback remediation since the eval gate is the arbiter those are tested against.
