...
| `eval/report.py` (102-108) | the migration recommendation is computed from `eval/scoring.py`'s keyword/word-count composite; the faithfulness-aware rubric judge is **advisory-only** (feeds the confidence label, not the pick) | **decides migration** | **CRITICAL — top open item** |
| `eval/scoring.py` (50) | "how good is this analysis?" by capitalization-regex + word count | judge *prompt* rebuilt, but scorer still drives the decision (row above) | PARTIAL |
...
1. **Inversion (CRITICAL).** `report.py:102-108` makes the migration recommendation from the deterministic `mean_composite` (a keyword/word-count proxy that `scoring.py:11-16` *admits* is "blind to faithfulness"); the rubric judge only feeds the confidence label (`_confidence`, 141-155) and cannot change the pick. A well-formatted, keyword-stuffing, *unfaithful* output clears the gate. **Fix:** make the judge's per-criterion verdict the primary signal in `summarize()`; demote the composite to a guardrail; fail-closed (not "tentative-but-recommended") when no judge signal exists.
2. **Three-way conflict of interest (CRITICAL).** By default `anchor == judge == reference == grok-4.3` (`harness.py:271-272`, `eval.py:148-150`, `judge.py:6-7`): the incumbent judges its own replacement, against its own output, as referee. "Conservative bias" is a euphemism for a one-directional *false-negative generator on migration* — it suppresses exactly the cheaper/local recommendations the eval exists to find, contradicting the local-first charter. **Fix:** default the judge to a family that is neither the anchor's nor any candidate's; if only the incumbent is available, degrade honestly to "no admissible head-to-head signal" rather than a caveated biased verdict; make `judge_shares_family` a hard confidence block in `report.py`, not a dim CLI note.
3. **Reference-against-incumbent circularity (HIGH).** Scoring the candidate on similarity to grok's output penalizes *different-but-better* and selects for grok-mimicry. **Fix:** score each output's faithfulness/coverage against the SOURCE absolutely; use pairwise-vs-anchor only as a secondary tiebreaker; drop the deterministic heuristic prior from the *gating* judge prompt so it stays independent of the brittle signal.
4. **No statistics (HIGH).** N=3 fixtures per workload (`fixtures.py:217-219`) drive a go/no-go via a bare `>=` on two point estimates, no CI, no significance test; win-rate is a mean of 2 deterministic comparisons; judge accuracy is never calibrated. **Fix:** paired bootstrap CI on (candidate − anchor) per fixture, require the lower bound to clear the bar; enforce a minimum N before "high" confidence; sample the judge k≥3 at temp>0 for a self-consistency band; **calibrate the judge against the golden/degraded pairs already in `golden.py`** (measure Cohen's kappa, *derive* the win-rate floor instead of hard-coding 0.45).
5. **Goodhart (HIGH).** The frozen, deterministic, transparent target rewards keyword-parroting + boilerplate formatting + grok-imitation; faithfulness (the rubric's top criterion) has zero weight on the pick. **Fix:** falls out of #1 — let faithfulness veto regardless of composite.

These supersede the staged P1-P4 above in priority: **the eval gate must be fixed first, because it is the arbiter every other remediation is "eval-gated" against.** An uncalibrated, incumbent-biased, faithfulness-blind gate cannot certify the other fixes.
