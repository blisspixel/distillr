...
| `eval/report.py` `summarize` | migration recommendation now gates on the faithfulness judge (composite is a guardrail floor; judge holds the veto; fail-closed without a neutral judge) | decides migration | **FIXED 2026-06-13** |
| `eval/scoring.py` (50) | "how good is this analysis?" by capitalization-regex + word count | demoted to the guardrail floor it was always meant to be; no longer the sole decider | DEMOTED |
...
5. **Goodhart (HIGH) — MITIGATED by #1.** The composite no longer decides alone, so keyword-stuffing/boilerplate can't license a migration without the judge confirming faithfulness. Residual: the composite still gates eligibility as a floor, so it should eventually be replaced by per-criterion judge verdicts (the AutoRubric shape) rather than substring counts.
