...
**Also recent: 0.9.6 `distill eval`** — a cost × quality model-selection harness with an advisory LLM-judge: sweep candidate models over frozen fixtures and get a deterministic recommendation for the cheapest model that clears your quality bar (the way to decide if a free local model can replace the grok-4.3 cloud floor). See [`docs/usage.md`](docs/usage.md#evaluate-models-cost--quality).
