...
class TestOpenVault:
    ...
    def test_vault_calls_webbrowser_open(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("distill._cli_impl.get_config", return_value=mock_config), \
            ...
            open_cmd(
                topic=None, channel=None, what="output",
                vault=True, path=""
            )

            mock_open.assert_called_once_with(str(library_dir))

    def test_vault_with_editor_env_var(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_vault_missing_library_dir(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_vault_with_path_option(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("distill._cli_impl.get_config", return_value=mock_config), \
            ...
            open_cmd(
                topic=None, channel=None, what="output",
                vault=True, path="topics/ai-agents"
            )

            mock_open.assert_called_once_with(str(subdir))

    def test_vault_with_missing_subdirectory(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("distill._cli_impl.get_config", return_value=mock_config), \
            ...
            with pytest.raises(click.exceptions.Exit):
                open_cmd(
                    topic=None, channel=None, what="output",
                    vault=True, path="nonexistent/path"
                )
