...
def test_private_attachment_helpers_cover_failure_paths(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "distill.site_attachments.requests.get",
    ...
    monkeypatch.setattr("distill.site_attachments.PdfReader", EmptyPdfReader)
    ...
    monkeypatch.setattr(
        "distill.site_attachments.get_transcript",
    ...
    monkeypatch.setattr("distill.site_attachments.get_transcript", lambda *_args, **_kwargs: False)
    failed_video, _ = _ingest_youtube_attachment(video_attachment, attachments_dir, config)
    assert failed_video.status == "failed"
