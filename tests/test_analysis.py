...
def test_analyze_video_builds_frontmatter(tmp_path):
    ...
    with patch("distill.analysis.llm_call", _fake_llm_call_sequence(["pass1 body", "pass2 body"])):
...
def test_analyze_short_marks_content_type(tmp_path):
    ...
    with patch("distill.analysis.llm_call", _fake_llm_call("short body")):
...
def test_generate_channel_context_delegates_to_router(tmp_path):
    ...
    with patch("distill.analysis.llm_call", _fake_llm_call("channel context")):
...
def test_analyze_scan_marks_scan_mode(tmp_path):
    ...
    with patch("distill.analysis.llm_call", _fake_llm_call("scan body")):
...
def test_generate_watch_instructions_delegates_to_router(tmp_path):
    ...
    with patch("distill.analysis.llm_call", _fake_llm_call("watch instructions")):
...
def test_analyze_video_tracker_records_tokens(tmp_path):
    ...
    with patch("distill.analysis.llm_call", _fake_llm_call_sequence(["p1", "p2"])):
