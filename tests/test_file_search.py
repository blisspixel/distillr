...
from distill.file_search import _gather_files, cleanup_stores, create_research_store
...
def test_create_research_store_returns_zero_when_no_files(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("distill.file_search._gather_files", lambda *args, **kwargs: [])
...
def test_create_research_store_uploads_and_waits_for_indexing(tmp_path, monkeypatch):
    config = DistillConfig(distill_output_dir=tmp_path / "lib")
    monkeypatch.setattr(
        "distill.file_search._gather_files",
        lambda *args, **kwargs: [("doc-1", "# One"), ("doc-2", "# Two")],
    )
    monkeypatch.setattr("distill.file_search.time.sleep", lambda seconds: None)
    observed_temp_paths = []
...
def test_delete_store_and_list_stores_handle_client_objects():
    ...
    from distill.file_search import delete_store, list_stores
...
def test_gather_files_includes_site_synthesis_and_page_insights(tmp_path):
    from distill.config import DistillConfig
    from distill.file_search import _gather_files
